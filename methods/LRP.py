...
class DTD(nn.Module):
    ...
    def forward(self, module_stack, y, class_num, model_archi):
        R = torch.eye(class_num)[torch.max(y, 1)[1]].cuda()
...
@torch.no_grad()
def dtd_for_vgg(model: nn.Module, inp, target, n_classes=1000, scale=5000):
    model = clone_freeze(model)
    dtd = DTD()
    store_model = ActivationStoringNet(model_flattening(model))
    stack, output = store_model(inp)
    one_hot = torch.zeros_like(output).to(inp.device)
    one_hot[0, target[0]] = 1
    saliency_map = dtd(stack, one_hot, n_classes, 'vgg')
    saliency_map = saliency_map.detach().permute((0, 2, 3, 1)).cpu().numpy()
    saliency_map = np.sum(saliency_map, axis=3)
    saliency_map = np.maximum(0, saliency_map) * 255 * scale
    saliency_map = np.minimum(255, saliency_map)
